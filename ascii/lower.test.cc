...
namespace snn::app
{
    namespace
    {
        ...
        {
            snn_require(ascii::lower("aBcDEF123") == "abcdef123");
            // Only ASCII characters are converted.
            snn_require(ascii::lower("aBcDEF123Åäö") == "abcdef123Åäö");

            str s;
            ascii::lower("aBc", s);
            ascii::lower("DEF123Åäö", s);
            snn_require(s == "abcdef123Åäö");

            return true;
        }

        constexpr bool test_lower()
        {
            str all;
            ...
            snn_require(ascii::lower("") == "");
            snn_require(ascii::lower("A") == "a");
            snn_require(ascii::lower("a") == "a");

            // Only A-Z gets lowercased.
            snn_require(
                ascii::lower(all) ==
                "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f\x10\x11\x12"
                "\x13\x14\x15\x16\x17\x18\x19\x1a\x1b\x1c\x1d\x1e\x1f "
            ...
            // UTF-8 (does nothing).
            snn_require(ascii::lower("Åäö") == "\xc3\x85\xc3\xa4\xc3\xb6");
            ...
            ascii::lower("VWXYZ0123456789", abc);
            snn_require(abc == "ABCDEFGHIJKLMNOPQRSTuvwxyz0123456789");
            snn_require(abc.capacity() > abc.default_capacity());

            // From self.
            ascii::lower(abc.view(), abc);
            snn_require(abc == "ABCDEFGHIJKLMNOPQRSTuvwxyz0123456789"
...
namespace snn
{
    void unittest()
    {
        ...
        snn_static_require(app::test_lower());
    }
}
