...
namespace snn::app
{
    namespace
    {
        constexpr bool example()
        {
            ...
            snn_require(l.count() == 10'000); // Clamped
...
namespace snn
{
    void unittest()
    {
        ...
        {
            constexpr auto ms_count = [](time::limit<time::milliseconds, 500, 10'000> l) {
                return l.count();
            };

            static_assert(ms_count(time::seconds{2}) == 2'000);
            static_assert(ms_count(time::milliseconds{100}) == 500); // Clamped
            static_assert(ms_count(time::seconds{12}) == 10'000);    // Clamped
        }
